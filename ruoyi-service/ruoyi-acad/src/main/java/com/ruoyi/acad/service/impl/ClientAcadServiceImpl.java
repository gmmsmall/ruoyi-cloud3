        ...
        if (clientSearchCriteria != null) {
            ...
            if (StringUtils.isNotBlank(clientSearchCriteria.getAcadName())) {
                boolQueryBuilder.must(QueryBuilders.multiMatchQuery(
                        clientSearchCriteria.getAcadName(), "name.cnName", "name.enName", "name.realName"));
            }

            //如果查询开始时间不为空
            if (StringUtils.isNotBlank(clientSearchCriteria.getStartBirthday())) {
                boolQueryBuilder.must(QueryBuilders.rangeQuery("baseInfo.brithday").gt(clientSearchCriteria.getStartBirthday()).lte(clientSearchCriteria.getEndBrithday()));
